...
class ForwardMessageUtil:
    ...
    def Save(self):
        ...
        for key in self.messages:
            values[key]=[x.jump_url if x != None else None for x in self.messages[key]]
        ...
        for key in values:
            for i in range(5):
                if values[key][i]!=None:
                    ...
                    message = await ForwardMessageUtil.MessageFromURL(guild,values[key][i])
    ...
    async def Forward(self,channel:discord.TextChannel,boss):
        if channel.guild.id not in self.messages or self.messages[channel.guild.id][boss] == None:
            return
        await self.messages[channel.guild.id][boss].forward(channel)
...
class RaiseHandCounter:
    ...
    def Count(self,message:discord.Message):
        if message == None or message.author.bot:
            return 0
        if (len(message.stickers) > 0 and message.stickers[0].id == 1304441542542360637) or "<:honsen_start:1343468869137858600>" in message.content or "<:seiken:806760893521985546>" in message.content:
            self.members[message.guild.id][message.channel.id]={}
            del self.messages[message.guild.id][message.channel.id]
            del self.edit_timer[message.guild.id][message.channel.id]
            return 0
        if message.guild.id in self.members and message.channel.id in self.members[message.guild.id] and message.author.id in self.members[message.guild.id][message.channel.id]:
            if (datetime.now()-self.edit_timer[message.guild.id][message.channel.id]).total_seconds() > 3600:
                return 0
            if self.members[message.guild.id][message.channel.id][message.author.id] == False:
                self.members[message.guild.id][message.channel.id][message.author.id] = True
                asyncio.create_task(self.Edit(message.channel))
                return sum(1 for x in self.members[message.guild.id][message.channel.id] if self.members[message.guild.id][message.channel.id][x])
        return 0
    async def Edit(self,channel:discord.TextChannel):
            if datetime.now() > self.edit_timer[channel.guild.id][channel.id]:
                self.edit_timer[channel.guild.id][channel.id] = datetime.now() + timedelta(seconds=7.1)
                await asyncio.sleep(7)
                self.edit_timer[channel.guild.id][channel.id] = datetime.now()
                
                text = discord.ui.TextDisplay(self.messages[channel.guild.id][channel.id].components[0].content)
                notready = discord.ui.Container(discord.ui.TextDisplay("⠀"))
                ready = discord.ui.Container(discord.ui.TextDisplay("⠀"),accent_color=discord.Colour.green())
                ...
                ready.children[0].content=f"{len(ready_members)}人\n"+" ".join(ready_members) if len(ready_members) > 0 else "0人"
                notready.children[0].content=" ".join(notready_members) if len(notready_members) > 0 else "⠀"
...
class TotsuAtSameTimeReservationData:
    ...
    def button_disabled(self,guild_id):
        ...
        for boss in list(itertools.chain.from_iterable(self.data[guild_id].combination)):
            result[boss-1]=True
        return result
...
class TotsuChangeView(discord.ui.View):
    def __init__(self,member_id,totsu:list):
        ...
        mochi=[int(str(totsu[5])[i:i+2]) for i in range(0,len(str(totsu[5])),2)] if totsu[5] != None else [0]
        for row in range(5):
            for column in range(5):
                ...
                self.add_item(TotsuChageButton(text,member_id,style,row,column,totsu))
...
class TotsuChageButton(discord.ui.Button):
    ...
    async def callback(self, interaction:discord.Interaction):
        mochi=[int(str(self.totsu[5])[i:i+2]) for i in range(0,len(str(self.totsu[5])),2)] if self.totsu[5] != None else [0]
        prev=mochi.copy()
        match self.row:
            case 0:
                if self.totsu[self.col] == None:
                    db.write_member(interaction.guild_id,self.member_id,f"boss{self.col+1}",1)
                else:
                    db.delete_member(interaction.guild_id,self.member_id,f"boss{self.col+1}")
            case 1:
        ...
        if mochi!=prev:
            ...
            if len(mochi)==0:
                ...
                db.write_member(interaction.guild_id,self.member_id,"mochi",sum([mochi[i]*(100**i) for i in range(len(mochi))]))
        totsu=db.execute(f"SELECT boss1,boss2,boss3,boss4,boss5,mochi FROM member_{interaction.guild_id} WHERE id = {self.member_id}")[0]
        content="１列目　本凸\n２列目　フル持ち越し\n３列目　長い持ち越し\n４列目以降　短い持ち越し"
        await interaction.response.edit_message(content=content,view=TotsuChangeView(self.member_id,totsu))
        message=interaction.guild.get_channel(db.read_guild("reaction_ch",interaction.guild_id)).get_partial_message(db.read_guild("reaction_msg",interaction.guild_id))
...
class RecruitMemberButton(discord.ui.Button):
    def __init__(self,boss_num):
        super().__init__(label=f"{boss_num}募集",style=discord.ButtonStyle.gray,custom_id=f"Recruitmember{boss_num}")
        self.boss_num=boss_num
    async def callback(self, interaction:discord.Interaction):
        members=db.execute(f"SELECT id FROM member_{interaction.guild_id} WHERE boss{self.boss_num} IS NOT NULL AND AFK IS NULL")
        members=list(itertools.chain.from_iterable(members))
        mochi=db.execute(f"SELECT id,mochi FROM member_{interaction.guild_id} WHERE mochi > 0 AND AFK IS NULL")
        for m in mochi:
            if str(self.boss_num) in [str(m[1])[i+1] for i in range(0,len(str(m[1])),2)]:
                members.append(m[0])
        if len(members) == 0:
            await interaction.response.send_message(f"{self.boss_num}ボスのメンバーはいません",ephemeral=True)
        else:
            if reservation.contain(interaction.guild_id,self.boss_num):
                await interaction.response.send_message(view=RecruitAtSameTimeView(interaction.guild,self.boss_num),ephemeral=True)
            else:
                members_name=[interaction.guild.get_member(m).mention for m in set(members)]
                await interaction.response.send_message(ephemeral=True,view=RecruitConfirmView(self.boss_num,members_name))
...
class RecruitConfirmView(discord.ui.LayoutView):
    ...
    async def main_button(self,interaction:discord.Interaction,button):
        ...
        send = await channel.send(view=RecruitMessageLayoutView(self.boss_num,self.members))
...
class RecruitMemberSelectButton(discord.ui.Button):
    def __init__(self):
        super().__init__(label="単騎",style=discord.ButtonStyle.gray,custom_id="Recruitmemberselectbutton",row=1)
    async def callback(self, interaction:discord.Interaction):
        await interaction.response.send_message("単騎募集するメンバーを選択",view=RecruitMemberSelectView(interaction.guild),ephemeral=True)
...
class RecruitMemberSelect(discord.ui.Select):
    def __init__(self,guild:discord.Guild):
        # 凸希望にあるメンバー一覧の取得
        members=db.execute(f"SELECT id FROM member_{guild.id} WHERE boss1 IS NOT NULL OR boss2 IS NOT NULL OR boss3 IS NOT NULL OR boss4 IS NOT NULL OR boss5 IS NOT NULL")
        mochi=db.execute(f"SELECT id,mochi FROM member_{guild.id} WHERE mochi > 0 AND AFK IS NULL")
        for m in mochi:
            members.append(m[0])
        ...
        if len(members)>=1:
            for i,m in enumerate(members):
                options.append(discord.SelectOption(label=f"{guild.get_member(int(m)).display_name}",value=m))
        max_values=20
...
class RecruitAtSameTimeView(discord.ui.LayoutView):
    def __init__(self,guild:discord.Guild,boss):
        ...
        self.member1,self.member2,self.common_sub,common=reservation.get_members(guild.id,boss,self.other,True)
        super().__init__(timeout=240)
        self.text.content=f"# {boss}ボス募集({len(self.member1)}人)\n{self.other}ボスと同時凸予定"
        self.container_boss1.add_item(discord.ui.TextDisplay(f"## {boss}ボス（{len(self.member1)}人）\n"+" ".join([f"<@{m}>" for m in self.member1])))
        self.container_boss2.add_item(discord.ui.TextDisplay(f"-# {self.other}ボス（{len(self.member2)}人）\n"+" ".join([f"<@{m}>" for m in self.member2])))
        ...
        if len(self.common_sub) > 0:
            ...
            container_common.add_item(discord.ui.TextDisplay(f"## 未振り分け\n"+" ".join([f"<@{m}>" for m in self.common_sub])))
            self.add_item(discord.ui.Separator())
            self.add_item(container_common)
            boss_list=sorted([boss,self.other])
            self.add_item(discord.ui.ActionRow(RecruitAtTheSameTimeDivideButton(boss,boss_list[0],boss_list[1],self.common_sub)))
    ...
    async def bosyu(self,interaction:discord.Interaction,button):
        members_name=[interaction.guild.get_member(m).mention for m in set(self.member1+self.common_sub)]
        await interaction.response.edit_message(view=RecruitConfirmView(self.boss,members_name))
    @action.button(label="同時凸募集",style=discord.ButtonStyle.green)
    async def doutotsu(self,interaction:discord.Interaction,button):
        await interaction.response.edit_message(view=RecruitAtSameTimeConfirmView(self.boss,self.other,self.member1+self.common_sub,self.member2+self.common_sub))
...
class RecruitAtSameTimeConfirmView(discord.ui.LayoutView):
    def __init__(self,boss1,boss2,members1,members2):
        ...
        text1=discord.ui.TextDisplay(f"## {boss1}ボス（{len(self.members_name1)}人）\n"+" ".join(self.members_name1))
        text2=discord.ui.TextDisplay(f"## {boss2}ボス（{len(self.members_name2)}人）\n"+" ".join(self.members_name2))
...
class RecruitAtTheSameTimeDivideButton(discord.ui.Button):
    ...
    async def callback(self, interaction:discord.Interaction):
        await interaction.response.edit_message(view=RecruitAtTheSameTimeDivideView(interaction.guild,self.select_boss,self.boss1,self.boss2,self.members))
class RecruitAtTheSameTimeDivideView(discord.ui.LayoutView):
    ...
    async def boss1_button(self,interaction:discord.Interaction,button):
        reservation.set_members(interaction.guild_id,[self.members[0]],[True],self.boss1)
        if len(self.members) > 1:
            await interaction.response.edit_message(view=RecruitAtTheSameTimeDivideView(interaction.guild,self.select_boss,self.boss1,self.boss2,self.members[1:]))
        else:
            view = discord.ui.LayoutView()
            view.add_item(discord.ui.TextDisplay("振り分けが完了しました"))
            await interaction.response.edit_message(view=view)
            await asyncio.sleep(0.5)
            await interaction.edit_original_response(view=RecruitAtSameTimeView(interaction.guild,self.select_boss))
    @action.button(style=discord.ButtonStyle.green,id=2)
    async def boss2_button(self,interaction:discord.Interaction,button):
        reservation.set_members(interaction.guild_id,[self.members[0]],[True],self.boss2)
        if len(self.members) > 1:
            await interaction.response.edit_message(view=RecruitAtTheSameTimeDivideView(interaction.guild,self.select_boss,self.boss1,self.boss2,self.members[1:]))
        else:
            view = discord.ui.LayoutView()
...
class RecruitMessageLayoutView(discord.ui.LayoutView):
    ...
    text=discord.ui.TextDisplay("募集")
    notready=discord.ui.Container(
        discord.ui.TextDisplay("0人")
    )
...
class ForwardMessageButton(discord.ui.Button):
    def __init__(self):
        super().__init__(label="転送設定",style=discord.ButtonStyle.gray,custom_id="forward_message",row=2)
    async def callback(self, interaction:discord.Interaction):
        await interaction.response.send_message(view=ForwardMessageSettingView(interaction.guild_id),ephemeral=True)
...
class ForwardMessageSettingContainer(discord.ui.Container):
    ...
    def __init__(self, guild_id,boss):
        ...
        if message != None:
            self.messagetext.content=message.content if message.content != '' else ' ⠀'
    ...
    async def change(self,interaction:discord.Interaction,button):
        await interaction.response.send_modal(ForwardMessageSettingModal(self.boss,interaction.guild_id))
        await interaction.delete_original_response()
    @actionrow.button(label="削除",style=discord.ButtonStyle.red)
    async def delete(self,interaction:discord.Interaction,button):
        await interaction.response.edit_message(view=ForwardMessageDeleteView(self.boss))
...
class ForwardMessageSettingModal(discord.ui.Modal):
    ...
    url= discord.ui.TextInput(
        label = "メッセージURL",
        placeholder="転送するメッセージのURL",
        required=False
    )
    async def on_submit(self, interaction:discord.Interaction):
        prev = forward_message.Get(interaction.guild_id,self.boss-1)
        if  prev == self.url.value:
            await interaction.response.send_message(f"設定は変更されませんでした\n{prev} → {self.url.value}",ephemeral=True,delete_after=10)
            return
        if self.url.value == "":
            forward_message.Delete(interaction.guild_id,self.boss-1)
            forward_message.Save()
            await interaction.response.send_message("メッセージの転送設定を取り消しました",ephemeral=True,delete_after=10)
            return
        message = await ForwardMessageUtil.MessageFromURL(interaction.guild,self.url.value)
        if message != None:
            forward_message.Register(message,self.boss-1)
            forward_message.Save()
            await interaction.response.send_message(f"転送するメッセージを変更しました\n{prev} → {self.url.value}",ephemeral=True,delete_after=10)
        else:
            await interaction.response.send_message(f"指定したメッセージの取得に失敗しました\n{self.url.value}",ephemeral=True,delete_after=10)
...
class ForwardMessageDeleteView(discord.ui.LayoutView):
    ...
    async def ok(self,interaction:discord.Interaction,button):
        ...
        forward_message.Save()
        view = discord.ui.LayoutView()
        view.add_item(discord.ui.TextDisplay(f"{self.boss}ボスの転送設定を削除しました"))
...
class ForwardMessageSettingAllContainer(discord.ui.Container):
    ...
    async def change(self,interaction:discord.Interaction,button):
        await interaction.response.send_modal(ForwardMessageSettingAllModal(interaction.guild_id))
        await interaction.delete_original_response()
...
class ForwardMessageSettingAllModal(discord.ui.Modal):
    ...
    url=discord.ui.TextInput(label="URL",placeholder="転送するメッセージのURL",required=False)
    async def on_submit(self, interaction:discord.Interaction):
        message = await ForwardMessageUtil.MessageFromURL(interaction.guild,self.url.value)
        if message != None:
            for i in range(5):
...
class TotsuAtTheSameTimeStartButton(discord.ui.Button):
    ...
    async def callback(self, interaction:discord.Interaction):
        await interaction.response.send_message(view=TotsuAtTheSameTimeView(interaction.guild),ephemeral=True)
...
class TotsuAtTheSameTimePreviewContainer(discord.ui.Container):
    ...
    def __init__(self,guild:discord.Guild,combination,idx):
        ...
        member1,member2,common_sub,common = reservation.get_members(guild.id,self.boss1,self.boss2)
        super().__init__(accent_color=discord.Color.from_rgb(255,255,int(255/self.boss1)))
    ...
    async def bosyu_button(self,interaction:discord.Interaction,button):
        members1,members2,common_sub,_=reservation.get_members(interaction.guild_id,self.boss1,self.boss2,True)
        await interaction.response.edit_message(view=RecruitAtSameTimeConfirmView(self.boss1,self.boss2,members1+common_sub,members2+common_sub))
    @action.button(label="編集",style=discord.ButtonStyle.green)
    async def edit_button(self,interaction:discord.Interaction,button):
        await interaction.response.edit_message(view=TotsuAtTheSameTimeSettingView(interaction.guild,self.boss1,self.boss2))
    @action.button(label="削除",style=discord.ButtonStyle.red)
    async def delete_button(self,interaction:discord.Interaction,button):
        await interaction.response.edit_message(view=TotsuAtTheSameTimeDeleteView(self.idx))
...
class TotsuAtTheSameTimeRegister(discord.ui.Container):
    ...
    async def register(self,interaction:discord.Interaction,button):
        await interaction.response.edit_message(view=TotsuAtTheSameTimeSelectBossView(interaction.guild_id))
    ...
    async def callback(self, interaction:discord.Interaction):
        if self.view.button_click(self.id):
            ...
            if reservation.register(interaction.guild_id,boss1,boss2):
                await interaction.response.edit_message(view=TotsuAtTheSameTimeSettingView(interaction.guild,boss1,boss2))
            else:
                self.view.selected=[]
                await interaction.response.send_message(f"{boss1}ボス、または{boss2}ボスは既に登録されているか凸するメンバーがいません",ephemeral=True)
        else:
...
class TotsuAtTheSameTimeSettingView(discord.ui.LayoutView):
    def __init__(self,guild:discord.Guild,boss1,boss2):
        ...
        self.guild=guild
        super().__init__(timeout=300)
        member1,member2,common_sub,common=reservation.get_members(guild.id,boss1,boss2)
        text1=discord.ui.TextDisplay(f"## {boss1}ボス\n"+" ".join([f"<@{m}>" for m in member1]))
        text2=discord.ui.TextDisplay(f"## {boss2}ボス\n"+" ".join([f"<@{m}>" for m in member2]))
        text3=discord.ui.TextDisplay(f"## 未振り分け\n"+" ".join([f"<@{m}>" for m in common_sub]))
        ...
        self.container_common.add_item(discord.ui.ActionRow(TotsuAtTheSameTimeDivideStartButton(boss1,boss2,common_sub)))
    
    def update(self):
        member1,member2,common_sub,common=reservation.get_members(self.guild.id,self.boss1,self.boss2)
        ...
        self.container_boss1.add_item(discord.ui.ActionRow(select1))
        self.container_boss2.add_item(discord.ui.ActionRow(select2))
        self.container_common.add_item(discord.ui.ActionRow(TotsuAtTheSameTimeDivideStartButton(self.boss1,self.boss2,common_sub)))
        return self
...
class TotsuAtTheSameTimeSettingSelect(discord.ui.Select):
    def __init__(self,members:list,selected:list,boss):
        ...
        self.boss=boss
        for i in range(len(members)):
            so = discord.SelectOption(label=f"{members[i].display_name}",default=selected[i],value=str(i))
            options.append(so)
        super().__init__(options=options,min_values=0,max_values=len(options),placeholder="凸するメンバーを選択")
    async def callback(self, interaction:discord.Interaction):
        selected = [True if str(i) in self.values else False for i in range(len(self.members))]
        reservation.set_members(interaction.guild_id,[m.id for m in self.members],selected,self.boss)
...
class TotsuAtTheSameTimeDivideStartButton(discord.ui.Button):
    ...
    async def callback(self, interaction:discord.Interaction):
        if len(self.members) == 0:
            await interaction.response.send_message("振り分けるメンバーがいません",ephemeral=True,delete_after=10)
        else:
            await interaction.response.edit_message(view=TotsuAtTheSameTimeDivideView(interaction.guild,self.boss1,self.boss2,self.members))
...
class TotsuAtTheSameTimeDivideView(discord.ui.LayoutView):
    ...
    async def boss1_button(self,interaction:discord.Interaction,button):
        reservation.set_members(interaction.guild_id,[self.members[0]],[True],self.boss1)
        if len(self.members) > 1:
            await interaction.response.edit_message(view=TotsuAtTheSameTimeDivideView(interaction.guild,self.boss1,self.boss2,self.members[1:]))
        else:
            view = discord.ui.LayoutView()
            view.add_item(discord.ui.TextDisplay("振り分けが完了しました"))
            await interaction.response.edit_message(view=view)
            await asyncio.sleep(1)
            await interaction.edit_original_response(view=TotsuAtTheSameTimeSettingView(interaction.guild,self.boss1,self.boss2))
    @action.button(style=discord.ButtonStyle.green,id=2)
    async def boss2_button(self,interaction:discord.Interaction,button):
        reservation.set_members(interaction.guild_id,[self.members[0]],[True],self.boss2)
        if len(self.members) > 1:
...
class Table(commands.Cog):
    def __init__(self,bot:commands.Bot):
        ...
        self.bot.tree.add_command(app_commands.ContextMenu(name="リアクションリセット",callback=self.member_reset_context))
        self.bot.tree.add_command(app_commands.ContextMenu(name="3凸完了",callback=self.member_complete_context))
        self.bot.tree.add_command(app_commands.ContextMenu(name="凸希望編集",callback=self.totsu_change_context))
    ...
    def CreateTable(self,guild_id:int,restype=0):
        ...
        value=[[],[],[],[],[],[],[],[]]
        afkvalue=[[],[],[],[],[],[],[],[]]
        totsu=db.execute(f"SELECT id,boss1,boss2,boss3,boss4,boss5,AFK,mochi FROM member_{guild_id}")
        ...
        boss_sort=sorted([(i,v) for i,v in enumerate(boss_counter)],key=lambda x:x[1],reverse=True)
        ...
        for _t in totsu_num:
            ...
            for i in range(0,len(str(_t[7])),2):
                addition=0 if _t[7]==0 else 10 if str(_t[7])[i]=="1" else 100 if str(_t[7])[i]=="2" else 1000 if str(_t[7])[i]=="3" else 0
                if addition > 0:
        ...
        data={'名前':values_str[7],'1ボス':values_str[0],'2ボス':values_str[1],'3ボス':values_str[2],'4ボス':values_str[3],'5ボス':values_str[4]}
    ...
    def CreateTable2(self,guild_id:int,restype=0):
        value=[[],[],[],[],[],[],[],[],[]]
        routes=db.execute(f"SELECT id,boss1,boss2,boss3,boss4,boss5,AFK,mochi,route FROM member_{guild_id} WHERE finish IS NULL")
        routes_num=[[v if v != None else 0 for v in m] for m in routes if (m[1:6] == (None,None,None,None,None) and m[7] == None) or m[6] != None]
        members=[]
        ...
        for _r in routes_num:
            ...
            for i in range(0,len(str(_r[7])),2):
                addition=0 if _r[7]==0 else 10 if str(_r[7])[i]=="1" else 100 if str(_r[7])[i]=="2" else 0
                if addition > 0:
        ...
        data={'名前':values_str[8],'1ボス':values_str[0],'2ボス':values_str[1],'3ボス':values_str[2],'4ボス':values_str[3],'5ボス':values_str[4]}
        df=pd.DataFrame(data)
        fig, ax=plt.subplots(figsize=(10,len(value[0])*0.8))
        ax.axis('off')
        tb=ax.table(cellText=df.values,
    ...
    async def reactions(self,ctx:commands.Context,arg1=None):
        message=ctx.message if ctx.message.reference==None else ctx.channel.get_partial_message(ctx.message.reference.message_id)
        if arg1!=None:
            try:
    ...
    async def reaction_add_table(self,payload:discord.RawReactionActionEvent):
        ...
        if payload.emoji.name in ["1⃣","2⃣","3⃣","4⃣","5⃣"] and payload.message_id == db.read_guild("reaction_msg",payload.guild_id) and not is_dayfirst:
            db.write_member(payload.guild_id,payload.user_id,"boss"+str(payload.emoji.name.encode("utf-8"))[2],1)
            asyncio.create_task(self.img_auto_reaction(payload.guild_id))
            asyncio.create_task(self.img2_auto_reaction(payload.guild_id))
        elif payload.emoji.name=="🪑" and payload.message_id == db.read_guild("reaction_msg",payload.guild_id):
            ...
            self.afk_members[payload.guild_id][payload.user_id]=int(datetime.now().timestamp())
        # 持ち越しよう
        elif payload.emoji.name in ["1⃣","2⃣","3⃣","4⃣","5⃣"] and (payload.message_id == db.read_guild("reaction_mochi_msg",payload.guild_id) or (payload.message_id == db.read_guild("reaction_msg",payload.guild_id) and is_dayfirst)):
            ...
            if addition in mochi_list:
                return
            db.write_member(payload.guild_id,payload.user_id,"mochi",mochi*100+addition)
            asyncio.create_task(self.img_auto_reaction(payload.guild_id))
            asyncio.create_task(self.img2_auto_reaction(payload.guild_id))
    ...
    async def reaction_update_table(self,payload:discord.RawReactionActionEvent):
        if payload.emoji.name == "〰" and payload.message_id == db.read_guild("reaction_msg",payload.guild_id):
    ...
    async def reaction_remove_table(self,payload:discord.RawReactionActionEvent):
        ...
        is_dayfirst=next_day.day == 1
        if payload.emoji.name in ["1⃣","2⃣","3⃣","4⃣","5⃣"] and payload.message_id == db.read_guild("reaction_msg",payload.guild_id) and not is_dayfirst:
            db.delete_member(payload.guild_id,payload.user_id,"boss"+str(payload.emoji.name.encode("utf-8"))[2])
            asyncio.create_task(self.img_auto_reaction(payload.guild_id))
        elif payload.emoji.name=="🪑" and payload.message_id == db.read_guild("reaction_msg",payload.guild_id):
        ...
        # 持ち越しよう
            ...
            db.write_member(payload.guild_id,payload.user_id,"mochi",sum(m*(100**i) for i,m in enumerate(reversed(mochi_list))))
            asyncio.create_task(self.img_auto_reaction(payload.guild_id))
    
    @app_commands.guild_only()
    async def totsu_change_context(self,interaction:discord.Interaction,member:discord.Member):
        roles=[interaction.guild.get_role(808240600691900417),interaction.guild.get_role(1292855894454964336),interaction.guild.get_role(1335210159735177297)]
        if db.read_member(member.guild.id,"id",member.id) == None:
            await interaction.response.send_message("メンバーではありません",ephemeral=True)
        elif not (set(roles) & set(interaction.user.roles) or interaction.user.id == member.id):
            await interaction.response.send_message("自分以外の凸希望は変更できません",ephemeral=True)
        else:
            totsu=db.execute(f"SELECT boss1,boss2,boss3,boss4,boss5,mochi FROM member_{member.guild.id} WHERE id = {member.id}")[0]
            content="１列目　本凸\n２列目　フル持ち越し\n３列目　長い持ち越し\n４列目以降　短い持ち越し"
            await interaction.response.send_message(content,ephemeral=True,view=TotsuChangeView(member.id,totsu))

    @commands.Cog.listener("on_message")
    async def afk_check(self,message:discord.Message):
        if message.channel.id in [1293578758741295207,1293579100056846409,1335059739410698344,1293579743681319023,1157527508615430163]:
            if message.guild != None and message.guild.id in self.afk_members and message.author.id in self.afk_members[message.guild.id]:
                date=datetime.fromtimestamp(self.afk_members[message.guild.id][message.author.id])
                if (datetime.now()-date).total_seconds() > 60:
                    pmessage=message.guild.get_channel(db.read_guild("reaction_ch",message.guild.id)).get_partial_message(db.read_guild("reaction_msg",message.guild.id))
    ...
    async def raiser_count(self,message:discord.Message):
        ...
        if counter > 0:
            if counter >= 10:
                await message.add_reaction(bytes.fromhex(f"3{int(counter/10)}efb88fe283a3").decode("utf-8"))
            await message.add_reaction(bytes.fromhex(f"3{counter%10}e283a3").decode("utf-8"))
    ...
    async def simg_slash(self,interaction:discord.Interaction):
        result=self.CreateTable(interaction.guild_id)
        if result!=None:
            await interaction.response.send_message(file=result,ephemeral=True,view=RecruitMemberView())
        else:
            await interaction.response.send_message("🤔",ephemeral=True)
    
    
    async def img_fixed_load(self):
        imgfixed_ids=db.execute("SELECT id,imgfixed_ch,imgfixed_msg FROM guild WHERE imgfixed_ch IS NOT NULL")
        for imgfixed_id in imgfixed_ids:
            try:
                message=await self.bot.get_channel(imgfixed_id[1]).fetch_message(imgfixed_id[2])
                self.guild_img[str(imgfixed_id[0])]={}
                self.guild_img[str(imgfixed_id[0])]["fixed_message"]=message
                self.guild_img[str(imgfixed_id[0])]["next"]=datetime.now()+timedelta(seconds=5)
                ...
                pass
        imgfixed2_ids=db.execute("SELECT id,imgfixed2_ch,imgfixed2_msg FROM guild WHERE imgfixed2_ch IS NOT NULL")
        for imgfixed2_id in imgfixed2_ids:
            try:
                message2=await self.bot.get_channel(imgfixed2_id[1]).fetch_message(imgfixed2_id[2])
                self.guild_img2[str(imgfixed2_id[0])]={}
                self.guild_img2[str(imgfixed2_id[0])]["message"]=message2
                self.guild_img2[str(imgfixed2_id[0])]["next"]=datetime.now()+timedelta(seconds=5)
    ...
    @app_commands.guild_only()
    async def reaction_reset(self,interaction:discord.Interaction,member:discord.Member=None):
        await interaction.response.defer(ephemeral=True)

        try:
            if member==None:
                await self.guild_reset(interaction.guild_id)
                await interaction.edit_original_response(content="リアクションをリセットしました")
            else:
                await self.member_reset(member)
                await interaction.edit_original_response(content=f"{member.display_name}のリアクションをリセットしました")
        except:
            await interaction.delete_original_response()
    ...
    async def on_message_complete(self,message:discord.Message):
        ...
        if message.channel.id==db.read_guild("kanryou_ch",message.guild.id):
            if re.search("(凸完|凸終|完凸)",message.content):
                ...
                if db.read_member(message.guild.id,"finish",message.author.id)==None:
                    finished=db.read_member(message.guild.id,"finish")
                    db.write_member(message.guild.id,message.author.id,"finish",message.id)
                    if re.search(r"\d+人目",message.content):
                        finish_num=int(re.sub(r"\D","",re.search(r"\d+人目",message.content).group()))
                    else:
                        finish_num=0
                    if finish_num!=len(finished)+1:
                        await message.add_reaction(bytes.fromhex(f"3{str(len(finished)+1)[0]}e283a3").decode("utf-8"))
                        if len(str(len(finished)+1)) ==2:
                            await message.add_reaction(bytes.fromhex(f"3{str(len(finished)+1)[1]}efb88fe283a3").decode("utf-8"))
                else:
                    db.write_member(message.guild.id,message.author.id,"finish",message.id)
                #asyncio.create_task(self.delete_mochikoshi(message.guild.id,message.author.id))

            elif message.guild.id not in [739872778395844650,731720074754523148,1276184060791750656] and re.search(r"\d",message.content):
                await self.member_reset(message.author)
                #await message.add_reaction("👍")
                ...
                if db.read_member(message.guild.id,"finish",message.author.id)=="":
                    ...
                    db.write_member(message.guild.id,message.author.id,"finish",message.id)
            try:
                totsu_message=message.guild.get_channel(db.read_guild("totsukanri_ch",message.guild.id)).get_partial_message(db.read_guild("totsukanri_msg",message.guild.id))
                await totsu_message.edit(embed=discord.Embed(color=discord.colour.parse_hex_number("ffffff"),title="凸管理",description=Generator.totsu_content(message.guild)),allowed_mentions=discord.AllowedMentions(users=False))
            except:
                pass
    ...
    @app_commands.guild_only()
    async def member_complete_context(self,interaction:discord.Interaction,member:discord.Member):
        await interaction.response.defer(ephemeral=True)
        if db.read_member(member.guild.id,"id",member.id) != None:
            if db.read_member(interaction.guild_id,"finish",member.id) == None:
                ...
                await interaction.edit_original_response(content=f"{member.display_name}を完了にしました")
            else:
                db.delete_member(interaction.guild_id,member.id,"finish")
                await interaction.edit_original_response(content=f"{member.display_name}を未完了にしました")
            try:
                totsu_message=interaction.guild.get_channel(db.read_guild("totsukanri_ch",interaction.guild_id)).get_partial_message(db.read_guild("totsukanri_msg",interaction.guild_id))
                await totsu_message.edit(embed=discord.Embed(color=discord.colour.parse_hex_number("ffffff"),title="凸管理",description=Generator.totsu_content(interaction.guild_id)),allowed_mentions=discord.AllowedMentions(users=False))
            except:
                pass
        else:
            await interaction.edit_original_response(content=f"{member.display_name}はメンバーではありません")


    @app_commands.guild_only()
    async def member_reset_context(self,interaction:discord.Interaction,member:discord.Member):
        await interaction.response.defer(ephemeral=True)
        if db.read_member(member.guild.id,"id",member.id) != None:
            await self.member_reset(member)
            await interaction.edit_original_response(content=f"{member.display_name}のリアクションをリセットしました")
    ...
    async def guild_reset(self,guild_id):
        ...
        try:
            message=self.bot.get_guild(guild_id).get_channel(db.read_guild("reaction_ch",guild_id)).get_partial_message(db.read_guild("reaction_msg",guild_id))
    ...
    async def member_reset(self,member:discord.Member):
        ...
        try:
            reaction_partial_message=member.guild.get_channel(reaction_ch).get_partial_message(reaction_msg)
            ...
            for guild_id in guild_ids:
                try:
                    if guild_id[2] != None:
                        ...
                        mochi_message=self.bot.get_guild(guild_id[0]).get_channel(guild_id[1]).get_partial_message(guild_id[2])
    ...
    async def img_emoji_show(self,message:discord.Message):
        if message.author.bot:
            return
        if message.guild.id in self.emj and self.emj[message.guild.id] != None and re.match(self.emj[message.guild.id],message.content):
            ...
            if result!=None:
                ...
                await message.add_reaction("🤔")
        # emj2
        if message.guild.id in self.emj2 and self.emj2[message.guild.id] != None and re.match(self.emj2[message.guild.id],message.content):
            ...
            if self.guild_img[str(guild_id)]["updated"]==False:
                ...
                if result!=None:
                    if "message" in self.guild_img[str(guild_id)]:
                        try:
                            message:discord.Message=self.guild_img[str(guild_id)]["message"]
                    ...
                    if "fixed_message" in self.guild_img[str(guild_id)]:
                        try:
                            message:discord.Message=self.guild_img[str(guild_id)]["fixed_message"]
    ...
    async def reactions_load(self):
        ...
        for table in tables:
            if table[0].startswith("member_"):
                ...
                message_ids=db.execute(f"SELECT reaction_ch,reaction_msg FROM guild WHERE id = {guild_id}")
                members=db.execute(f"SELECT id,boss1,boss2,boss3,boss4,boss5,AFK,mochi FROM {table[0]}")
                self.afk_members[guild_id]={v[0]:v[6] for v in filter(lambda x:x[6] != None,members)}
                try:
                    message=await self.bot.get_guild(guild_id).get_channel(message_ids[0][0]).fetch_message(message_ids[0][1])
                    for reaction in message.reactions:
                        if reaction.emoji not in ["1⃣","2⃣","3⃣","4⃣","5⃣","🪑"]:
                            continue
                        reaction_member_ids=[user.id async for user in reaction.users()]
                        for member in members:
                            if reaction.emoji=="1⃣":
                                member_update(member[0],member[1],"boss1",member[7],reaction_member_ids)
                            if reaction.emoji=="2⃣":
                                member_update(member[0],member[2],"boss2",member[7],reaction_member_ids)
                            if reaction.emoji=="3⃣":
                                member_update(member[0],member[3],"boss3",member[7],reaction_member_ids)
                            if reaction.emoji=="4⃣":
                                member_update(member[0],member[4],"boss4",member[7],reaction_member_ids)
                            if reaction.emoji=="5⃣":
                                member_update(member[0],member[5],"boss5",member[7],reaction_member_ids)
                            if reaction.emoji=="🪑":
                                a=member_update(member[0],member[6],"AFK",member[7],reaction_member_ids)
                                if a != None:
                                    if a:
    ...
    async def img2_auto_reaction(self,guild_id):
        if str(guild_id) in self.guild_img2:
            ...
            if self.guild_img2[str(guild_id)]["updated"]==False:
                self.guild_img2[str(guild_id)]["next"]=datetime.now()+timedelta(seconds=10)
                self.guild_img2[str(guild_id)]["updated"]=True
                result=self.CreateTable2(guild_id,1)
                if result!=None:
                    if "message" in self.guild_img2[str(guild_id)]:
                        try:
                            message:discord.Message=self.guild_img2[str(guild_id)]["message"]
                            buffs=copy.copy(result)
                            attachment=discord.File(filename="table.png",fp=buffs)
                            await message.edit(attachments=[attachment])
                        except:
